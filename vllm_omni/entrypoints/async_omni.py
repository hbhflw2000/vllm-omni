...
class AsyncOmni(EngineClient, OmniBase):
    ...
    async def generate(
        ...
        **Batch mode (diffusion only):**
        ...
        sleeping_tags = getattr(self, "_sleeping_tags", None)
        if sleeping_tags:
            raise RuntimeError(
                "Generation rejected: Engine is partially or fully asleep. "
                f"Currently sleeping tags: {list(sleeping_tags)}. "
    ...
    async def wake_up(self, stage_ids: list[int] | None = None, tags: list[str] | None = None) -> list[OmniACK]:
        ...
        current_tags = getattr(self, "_sleeping_tags", set())
        if tags is None:
            requested_tags = list(current_tags)
        else:
            requested_tags = [tag for tag in tags if tag in current_tags]
        ...
        for tag in requested_tags:
            if hasattr(self, "_sleeping_tags"):
                self._sleeping_tags.discard(tag)
